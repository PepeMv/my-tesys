...
import { obtenerPedidosParaPrepararAction } from "../../../actions/pedidosActions";
...
const MostrarOrdenesLive = () => {
  ...
  const ordenesState = useSelector(
    (state) => state.pedidos.pedidosParaPreparar
  );
  const detallesState = useSelector(
    (state) => state.pedidos.detallesParaPreparar
  );
  ...
  return <AppFrame titulo="Pedidos para Preparar" body={renderBody()} />;
};

export default MostrarOrdenesLive;
