...
const MostrarOrdenesEntregar = () => {
  ...
  useEffect(() => {
    const traerPedidosEntregar = () =>  dispatch( obtenerPedidosParaEntregarAction() );
    traerPedidosEntregar();    
  }, []);

  const loading = useSelector((state) => state.pedidos.loading);
  const ordenesState = useSelector((state) => state.pedidos.pedidosParaEntregar);
  const detallesState = useSelector((state) => state.pedidos.detallesParaEntregar);
  ...
  return (
    <AppFrame titulo="Pedidos para Entregar" body={renderBody()} />
  );
};
